...
def make_planet(x_pos, y_pos, mass, colour, size, name): return (x_pos, y_pos, mass, colour, size, name)
def get_x_coordinate(planet): return planet[0]
def get_y_coordinate(planet): return planet[1]
def get_mass(planet): return planet[2]
def get_colour(planet): return planet[3]
def get_size(planet): return planet[4]
def get_name(planet): return planet[5]
def get_position(planet): return (get_x_coordinate(planet), get_y_coordinate(planet))
...
def setup_spacecraft(vx, vy, f):
    ...
    # Initial animation state & indicators
    pause = False
