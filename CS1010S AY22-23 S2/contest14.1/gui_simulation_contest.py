import re
import animation
import hungry_games_classes_contest as hgc
import cocos
import random
...
class GameReplayLayer(cocos.layer.Layer):
    ...
    def should_replay_event(event):
        return event[0] in ['MOVE', 'FMOVE', 'ATTACK', 'TOOK', 'KILLED', 'SPAWNED', 'ATE']

    def replay_event(self, event):
        handlers = {
            'MOVE': self.replay_move_event,
            'FMOVE': self.replay_move_event,
    ...
    def replay_eat_event(self, event):
        ...
        new_hp = min(200, event[1].get_health() + event[2].get_food_value())
