...
class Tribute(Person):
    ...
    def add_health(self, health):
        self.health = min(200, self.health+health)
    ...
    def attack(self, target, weapon):
        if weapon in self.get_weapons():
            if isinstance(target, LivingThing) and self.check_ranged_weapon(weapon, target):
                dmg = weapon.damage()
    ...
    def check_ranged_weapon(self, weapon, target):
        check = False
        # RangedWeapon can attack targets at neighbouring cells, not just the ones at the same cell
        if isinstance(weapon, RangedWeapon):
            check |= target.get_place() in self.get_place().get_neighbors()
        # Regular Weapon behaviour
        if isinstance(weapon, Weapon):
            check |= target.get_place() is self.get_place()
        return check
...
class GameLogger(object):
    ...
    def print_event(self, event, *args):
        stringify = {
            ...
            'FMOVE': lambda args: "{} forcefully moved from {} to {}".format(*(named_col(args))),
