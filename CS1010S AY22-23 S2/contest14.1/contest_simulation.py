...
class Match(object):
    ...
    def print_scores(self):
        print('===== Scores =====')
        for trb, score in self.scores.items():
            print(f'{trb.name}: {score}')
...
class Round(object):
    ...
    def gui_simulate(self):
        if not self.simulated:
            self.simulate()
        gui_simulation_contest.animate_game_events(self.game_map.layout, self.events, self.map_states)
    ...
    def json_output(self):
        ...
        def json_config():
            def json_map():
                return {'rows': self.game_map.rows, 'columns': self.game_map.columns, 'wrap': self.game_map.wrap == 1}
